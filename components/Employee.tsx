                ...
                <div className="flex lg:w-5/6 w-full px-8 flex-col mx-auto sm:space-x-4 sm:space-y-0 space-y-4 sm:px-0 items-end">
                  {data.map((item, index) =>(
                    <div className="relative flex-grow w-full px-2" key={item.value}>
                      ...
                      <input type={item.type} id={item.value} name={item.value} defaultValue={initialdata[index]} className="w-full bg-gray-200 rounded border border-gray-300 focus:border-indigo-500 focus:bg-transparent focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out"/>
                    </div>
                  ))}
                    <div className="relative flex-grow w-full px-2">
                      <select name="head" id="leader" className="w-full bg-gray-200 rounded border border-gray-300 focus:border-indigo-500 focus:bg-transparent focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
                        <option value="true">Leader of The Department</option>
                        <option value="false">Not a Leader</option>
                      </select>
                    </div>
