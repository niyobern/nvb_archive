...
export const getServerSideProps = handle({
  async get({ cookies }) {
    ...
    const leavetypes = await axios.get(`${baseUrl}/general/staff/promotion`, {headers: {"Authorization": token}})
    return json({data: leavetypes.data})
  },
  async post({req: { body }, cookies}){
    const token  = cookies.get('token')
    const result = await axios.post(`${baseUrl}/general/staff/promotion`, body, {headers: {"Authorization": token}})
    const data = result.data
    return json({message: data})
  }
});

export default function staffPromotion({ data }: any) {
