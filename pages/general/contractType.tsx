...
export const getServerSideProps = handle({
  async get({ cookies }) {
    ...
    const leavetypes = await axios.get(`${baseUrl}/general/contract/types`, {headers: {"Authorization": token}})
    return json({data: leavetypes.data})
  },
  async post({req: { body }, cookies}){
    const token  = cookies.get('token')
    const result = await axios.post(`${baseUrl}/general/contract/types`, body, {headers: {"Authorization": token}})
    const data = result.data
    return json({message: data})
  }
});

export default function ContractTypes({ data }: any) {
