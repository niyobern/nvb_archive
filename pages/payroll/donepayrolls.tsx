...
export const getServerSideProps = handle({
  async get({ cookies }) {
    ...
    const leavetypes = await axios.get(`${baseUrl}/payroll/done`, {headers: {"Authorization": token}})
    console.log(leavetypes.data)
    return json({data: leavetypes.data})
  },
  async post({req: { body }, cookies}){
    const token  = cookies.get('token')
    const result = await axios.post(`${baseUrl}/payroll/done`, body, {headers: {"Authorization": token}})
    const data = result.data
    return json({message: data})
  }
});

export default function DonePayroll({ data }: any) {
