...
export const getServerSideProps = handle({
  async get({ cookies }) {
    ...
    const leavetypes = await axios.get(`${baseUrl}/payments/pending`, {headers: {"Authorization": token}})
    console.log(leavetypes.data)
    return json({data: leavetypes.data})
  },
  async post({req: { body }, cookies}){
    const token  = cookies.get('token')
    const result = await axios.post(`${baseUrl}/payments/pending`, body, {headers: {"Authorization": token}})
    const data = result.data
    return json({message: data})
  }
});

export default function PendingPayments({ data }: any) {
