...
export const getServerSideProps = handle({
  async get({ cookies }) {
    ...
    const leavetypes = await axios.get(`${baseUrl}/administration/service/codes`, {headers: {"Authorization": token}})
    return json({data: leavetypes.data})
  },
  async post({req: { body }, cookies}){
    const token  = cookies.get('token')
    const result = await axios.post(`${baseUrl}/administration/services/codes`, body, {headers: {"Authorization": token}})
    const data = result.data
    return json({message: data})
  }
});

export default function serviceCodes({ data }: any) {
...
return (
    <Layout items={items} links={links}>
        <Table items={data ? data : mockData}/>
    </Layout>
)
